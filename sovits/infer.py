...
class Svc(object):
    ...
    def load_model(self, model_path):
        ...
        _ = self.n_g_ms.eval().to(dev)

    def get_unit_pitch(self, audio, sr, tran):
        audio = torchaudio.functional.resample(audio, sr, 16000)
        if len(audio.shape) == 2 and audio.shape[1] >= 2:
            audio = torch.mean(audio, dim=0).unsqueeze(0)
        soft = get_units(audio, self.hubert_soft).squeeze(0).cpu().numpy()
        input_pitch = transcribe(audio.cpu().numpy()[0], 16000, soft.shape[0], tran, self.feature_input)
    ...
    def infer(self, model_input_audio, model_input_sr, speaker_id, tran):
