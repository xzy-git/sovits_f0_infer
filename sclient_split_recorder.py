...
def ste(cur_frame):
    # 短时能量
    amp = np.sum(np.abs(cur_frame))
    return amp


def save_audio(namer_num, audio_2, audio_format, frames):
    wf = wave.open("./wav_temp/record/" + str(namer_num) + ".wav", 'wb')
    wf.setnchannels(1)
    wf.setsampwidth(audio_2.get_sample_size(audio_format))
