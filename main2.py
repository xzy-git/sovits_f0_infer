import logging
import os
import subprocess

import demjson
import soundfile
import torch
import torchaudio

from sovits import infer
from sovits.infer import Svc
from wav_temp import merge

logging.getLogger('numba').setLevel(logging.WARNING)
dev = torch.device("cuda" if torch.cuda.is_available() else "cpu")

model_name = "152_epochs.pth"  # 模型名称（pth文件夹下）
config_name = "nyarumul.json"
svc_model = Svc(f"pth/{model_name}", f"configs/{config_name}")
infer.mkdir(paths=["./raw", "./pth", "./results"])

clean_names = ["十年"]
trans = [0]
id_list = [0]
input_wav_path = "./wav_temp/input"
out_wav_path = "./wav_temp/output"

speakers = demjson.decode_file(f"configs/{config_name}")["speakers"]
infer.fill_a_to_b(a=trans, b=clean_names)
infer.mkdir(paths=[input_wav_path, out_wav_path])
print("mis连续超过10%时，考虑升降半音\n")
for clean_name, tran in zip(clean_names, trans):
    raw_audio_path = f"./raw/{clean_name}.wav"
    svc_model.format_wav(audio_path=raw_audio_path)
    for spk_id in id_list:
        # 清除缓存文件
        infer.del_temp_wav(path_data="./wav_temp")
        var_list = []
        mis_list = []
        out_audio_name = model_name.split(".")[0] + f"_{clean_name}_{speakers[spk_id]}"

        proc = subprocess.Popen(
            f"python ./sovits/slicer.py {raw_audio_path} --out_name {out_audio_name} --out {input_wav_path}  --db_thresh -30",
            shell=True).wait()
        # shutil.copy(raw_audio_path, f"{input_wav_path}/{out_audio_name}-00.wav")
        count = 0
        file_list = os.listdir(input_wav_path)
        len_file_list = len(file_list)
        for file_name in file_list:
            raw_path = f"{input_wav_path}/{file_name}"
            out_path = f"{out_wav_path}/{file_name}"

            raw_audio, raw_sr = torchaudio.load(raw_path)
            out_audio, out_sr = svc_model.infer(raw_audio, raw_sr, speaker_id=spk_id, tran=tran)
            soundfile.write(out_path, out_audio, svc_model.target_sample)

            mistake, var = svc_model.calc_error(in_path=raw_path, out_path=out_path, tran=tran)
            mis_list.append(mistake)
            var_list.append(var)
            count += 1
            print(f"{file_name}: {round(100 * count / len_file_list, 2)}%  mis:{mistake} var:{var}")
        print(
            f"分段误差参考：0.3优秀，0.5左右合理，少量0.8-1可以接受\n若偏差过大，请调整升降半音数；多次调整均过大、说明超出歌手音域\n半音偏差：{mis_list}\n半音方差：{var_list}")
        merge.run(out_audio_name)
        # 清除缓存文件
        infer.del_temp_wav("./wav_temp")
